...
visualizer = ClusterBathVisualizer((1, 4), 4, spacing=1, conceptual_baths=True)

# Modify sizes directly
visualizer.font_size = 12
visualizer.cluster_node_size = 0.1
visualizer.conceptual_bath_size = 0.125
...
visualizer.set_bath_count(0, 2)
visualizer.set_bath_count(1, 2)
visualizer.set_bath_count(2, 2)
visualizer.set_bath_count(3, 2)
...
visualizer.set_link_style(0, 1, color="#B19CD9", label=r"$\boldsymbol{\tilde{\theta}}^{A}$", style='dashed')
visualizer.set_link_style(0, 2, color="#E6D08A", label=r"$\boldsymbol{\theta}^{B}$", style='dashed')
visualizer.set_link_style(0, 3, color="#D4A5A5", label=r"$\boldsymbol{\tilde{\theta}}^{B}$", style='dashed')

visualizer.set_link_style(3, 0, color="#8BB68A", label=r"$\boldsymbol{\theta}^{A}$", style='dashed')
visualizer.set_link_style(3, 1, color="#B19CD9", label=r"$\boldsymbol{\tilde{\theta}}^{A}$", style='dashed')
visualizer.set_link_style(3, 2, color="#E6D08A", label=r"$-\boldsymbol{\theta}^{B}$", style='dashed')
visualizer.set_link_style(3, 3, color="#D4A5A5", label=r"$-\boldsymbol{\tilde{\theta}}^{B}$", style='dashed')
