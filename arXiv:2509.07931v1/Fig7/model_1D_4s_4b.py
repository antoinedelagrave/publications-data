...
def cdmft(model, varias, sub_baths_weights: list, session_id: str) -> pyqcm.model_instance:
    solution = pyqcm.cdmft.CDMFT(model,
                                 varias,
                                 accur_bath=1e-5,
                                 accur_dist=1e-12,
                                 method='bobyqa',
...
def main(run_cdmft: bool = False, sub_baths_weights: list = [], session_id: str = '') -> None:
    ...
    if run_cdmft:
        try:
            ...
            analyzer.plot_density_and_spectrum(
                ...
                labels=['Lieb-Wu', r'$N_{\mathrm{b}}=8$, $N_{\mathrm{sb}}=2$', r'$N_{\mathrm{b}}=8$, $N_{\mathrm{sb}}=1$'],
                save='./output/lieb-wu.pdf'
            )
